...
SECRET="${RUNTIME_SECRET:-test-secret}"
...
"$DOCKER_BIN" run -d \
  ...
  --name "$NAME" \
  -p 17080:7080 \
  -e SLACK_BOT_TOKEN="${SLACK_BOT_TOKEN:-xoxb-dummy}" \
  -e SLACK_APP_TOKEN="${SLACK_APP_TOKEN:-xapp-dummy}" \
...
assert request["authorization"] == "Bearer test-secret", request
